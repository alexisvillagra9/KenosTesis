...
namespace KenosTesis
{
    public partial class RegistrarProfesor : Form
    {
        ...
        private void RegistrarProfesor_Load(object sender, EventArgs e)
        {
            ...
            inicial();
            // TODO: esta línea de código carga datos en la tabla 'pilarSportClubDataSet12.sexo' Puede moverla o quitarla según sea necesario.
            this.MaximizeBox = false;
            // TODO: esta línea de código carga datos en la tabla 'pilarSportClubDataSet10.buscarProfesores' Puede moverla o quitarla según sea necesario.
            conexion = new SqlConnection("Data Source=DESKTOP-TGHGHKS;Initial Catalog=pilarSportClub;Integrated Security=True");
            adaptador = new SqlDataAdapter();
            adaptador.SelectCommand = new SqlCommand("buscarProfesor", conexion);
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                else
                {
                    if (dataGridView1.Rows[i].Cells[0].Value.Equals(true))
                    {
                        ...
                        SqlCommand consulta = new SqlCommand("select * from dbo.profesor where idProfesor=@ip", conexion);
                        adaptador.SelectCommand = consulta;
                        adaptador.SelectCommand.Parameters.Add(new SqlParameter("@ip", SqlDbType.Int));
                        datoObj = new DataSet();
                        conexion.Open();
                        adaptador.SelectCommand.Parameters["@ip"].Value = dataGridView1.Rows[dataGridView1.CurrentCellAddress.Y].Cells[1].Value;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (estBoton == "Guardar")
            {
                SqlCommand modifica = new SqlCommand("update profesor set nombreProfesor=@nom, apellidoProfesor=@ape, fechaNacimiento=@fec, localidad=@loc, telefono=@tel, dni=@dni, sexo=@sex, direccion=@dir where idProfesor=@id", conexion);
                ...
                MessageBox.Show("Se modifico correctamente el Profesor " + textBox1.Text + " " + textBox2.Text);
                conexion.Close();
            }
            if (estBoton == "Registrar")
            {
                RegistrarCategoria rc = Owner as RegistrarCategoria;
                SqlCommand alta = new SqlCommand("insert into profesor values (@nom, @ape, @fec, @dni, @sex, @tel, @dir, @loc)", conexion);
                ...
                conexion.Close();
                MessageBox.Show("Profesor Registrado Correctamente con el ID: " + consultaID(), "Alerta", MessageBoxButtons.OK);
                if (flag==1)
                {
                    rc.Show();
                    flag = 0;
        ...
        {
            DataSet datoObj1;
            SqlCommand consulta = new SqlCommand("select idProfesor from dbo.profesor where dni=@doc", conexion);
            adaptador.SelectCommand = consulta;
        ...
        {
            SqlCommand eliminar = new SqlCommand("delete from profesor where idProfesor=@idp", conexion);
            adaptador.DeleteCommand = eliminar;
            adaptador.DeleteCommand.Parameters.Add(new SqlParameter("@idp", SqlDbType.Int));
            conexion.Open();
            adaptador.DeleteCommand.Parameters["@idp"].Value = dataGridView1.Rows[dataGridView1.CurrentCellAddress.Y].Cells[1].Value;
